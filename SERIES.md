...
**這是依現有 Day 1 Repo 新擬的 Day 02–30 規劃，不是宣稱已找回舊對話完整目錄，也不是已完成的文章或功能。** 基準 `main` commit：`9185d18c1ce87d93574fe3fe45bb6f1e9a3617ea`。現況、已回收的首篇歷史網址與發布證據見 [STATUS.md](STATUS.md)。
...
五項跨篇交織，不強迫各占六天，也不是執行順序。

「會查、會做、會記，也會說不知道。」是產品目標；「AI 說『完成』，不等於後端真的完成。」是待實作驗證的工程原則。既有 `docs/day01/handoff-timeout-001.json` 保持不變，Day 14 將它接成測試，Day 21、29、30 再核對。

## 3. 範圍與技術門檻

Repo 已提出 Gemini API／Google AI Studio、Google ADK、Google Antigravity 與 Google Cloud 的方向，但尚未鎖定語言、SDK、模型、資料庫或部署版本。

本版建議先走單一語言、單一 Agent、小型合成資料與明確工具契約。Python 是候選，不是已批准或已存在的實作；Cloud Run／Firestore 是後段候選。Day 3 與 Day 24 各自完成選型與驗證，記入 [DECISIONS.md](DECISIONS.md)。不混用 Gemini API 世代或不同 ADK 版本，不憑記憶填方法名稱。

優先完成只讀查詢、有效確認、一次受控請求、逾時核對與可重現測試。多 Agent、MCP、向量資料庫、語音、付款與多租戶後台，只在確實解決必要問題時才提出變更，不為展示工具而加入。

ChatGPT、Codex、Antigravity 是可替換的開發協作工具，不是產品 runtime 的必要依賴。前期本機／隔離開發頻道的測試，不等於 Day 24 的雲端部署；部署與真實外部呼叫須另有帳號、預算及授權。

## 4. 30 天地圖

Day 1 已由作者確認開賽；歷史網址已回收於 STATUS，但本次未重新取得首篇全文，因此完整文章標題、末尾預告與平台計數仍待核對。不重新生成一份 Day 1 冒充已發布稿。下表 Day 02–30 全部是本版規劃。
...
| Day 21–26 | 評測、觀察、成本、雲端與變更流程 | 固定評測集、實測紀錄、部署驗證、停止與回滾演練 |
| Day 27–30 | 可移植、可重現、可交接 | 第二合成場域、乾淨環境、端到端比較與公開證據索引 |

測試從第一個實作開始，Day 21 是整合評測，不是首次測試。先處理 Day 02–04，再完成 Day 05–07；後續以 3–5 篇為一批。維持至少 3 天、目標 7 天的**連續可發布庫存**，再往 20 篇目標推進。這是工作目標，不是目前已有存稿數。

骨架、完整初稿、已驗證、作者核准可發布、iThome 已公開、平台計數已核對，是不同狀態。其他個人行程不寫入公開 Repo，只提前保護忙碌日期。

技術受阻時先修最短關鍵路徑與縮小非必要功能。設計篇可以交付完整且經審閱的規格，但必須標示尚未實作；不能發布空白骨架或把待測寫成通過。要換題／調序，先列出下游依賴，再同步本表與文章。

## 6. 逐篇交付規則

每篇包含本日新增能力、前置依賴、問題、設計、預定交付檔案、正向／反向案例、驗證方法、限制、配圖計畫、來源與前後銜接。預定檔案路徑不是已存在資產。

程式集中維護共用 `src/`、`tests/`、`examples/`，需要時才建資料夾，不每天複製整份程式。當日證據規劃放 `docs/dayNN/verification.md`，圖放 `assets/dayNN/`。證據帶執行版本／commit、環境、命令、輸入、預期、實際、判定及限制；官方格式與 LOCAL 內部格式分開。

每日驗證 → 作者審稿 → 發布 iThome → 開公開頁檢查正文／圖／程式／系列 → 核對 Day 計數 → 回填 STATUS。GitHub commit 不是 iThome 發布；排程不能未經查核就當成成功。此 Repo 不含自動發布設定。

## 7. 官方查證入口

以下是 2026-09-16 建稿時的查證起點，不表示每篇技術主張都已核對或測試。已取得的文件只提供方向，實作時仍須核對 API 世代、SDK／模型相容性與具體限制。引用要補實際章節、支持主張與查證日期。
...
賽事細則入口：[2026 iThome 鐵人賽活動頁](https://ithelp.ithome.com.tw/2026ironman/event)。本次未取得完整細則，AI 使用、每日計數、時區邊界與定時發布規則不能自行補造；由當屆官方規則／作者可見後台證據補存。文件中的備稿門檻是內部流程，不是假稱官方規定。

## 8. 本次來源界線

已讀：`README.zh-TW.md`、Repo 目錄、`docs/day01/handoff-timeout-001.json`，基準 commit 如頁首。作者已確認開賽；首篇原文尚未歸檔不等於懷疑作者未開賽。歷史回收 URL 與本次已重驗公開頁是不同證據層級，詳見 STATUS。

未取得舊串完整 30 天目錄，所以新增規劃使用本版號；與公開 Day 1 預告核對後再鎖定逐日標題。取得更完整的作者定稿時，列出差異、確認影響並更新，不默默覆寫既有決策。
