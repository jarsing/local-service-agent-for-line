...
正式文章：https://ithelp.ithome.com.tw/articles/10415219
...
- **等待中換版防禦**：使用者在畫面看到舊時段（07:30~11:00），若在決定期間系統已採用新版（08:00~11:00），按鈕送出的舊確認將被判定為 `version_changed`（舊確認不接受；新版內容需重新出示與核對），避免誤認舊資訊。
...
- `adk_bridge.py`：ADK 工具確認介接層，將 `ToolContext.request_confirmation` 與 `ConfirmationStore` 串接，提供 `lookup_confirmation_id_from_session` 避免回退至最新單。
